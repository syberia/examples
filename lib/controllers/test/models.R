...
preprocessor <- function() {
  ...
  colored_name <- function(color = 'red')
    sQuote(director:::colourise(tested_resource, color))
  ...
  output <- tryCatch(error = identity, stagerunner$run(verbose = TRUE, remember = FALSE))
  if (is(output, 'error')) {
    sink()
    cat("\nError when testing ", colored_name(), ": \n",
        paste(collapse = "\n", readLines(stream)), "\n\n",
        director:::colourise(output$message, 'red'), "\n")
    stop("Error when testing ", colored_name(), call. = FALSE)
  }
